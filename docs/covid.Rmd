...
utk_plot <- ggplot(utk.cases, aes(x=date, y=count, group=group)) + geom_smooth(aes(colour=group)) + geom_point(aes(colour=group)) + ylab("Number of active cases at UTK") + xlab("Date") + ylim(0,NA) + scale_colour_viridis_d(end=0.8)
print(utk_plot)
```
