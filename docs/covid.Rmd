...
We can also use the UTK saliva data to estimate the number of new cases: the proportion of students who ultimately test postive out of those tested by the saliva scans. Given poor compliance, the true proportions may be much higher than this (if students want to endanger others by not isolating, they may avoid testing if they think they have been exposed). Depending on the date of the test, between `r paste(paste("1 of every ", range(round(1/(saliva_data$Positive.diagnostic.tests./saliva_data$Samples.processed))), sep=""), collapse=" to ")` of students who contributed samples had active covid infections. 

I am converting the saliva test info to new cases per 100K to correspond with the [Harvard Global Health Institute's](https://globalepidemics.org/key-metrics-for-covid-suppression/) standards for guidance of control. However, these guidelines are for daily new cases but the saliva data are active cases. I am using the assumption that Knox County uses that new cases are only active for 14 days, so I'm computing the new case estimate as 1/14 the active case estimate. The green, yellow, and orange bands are the same as for Knox and Anderson+Roane counties above; the red band looks much thicker because it has the same lower bound as the others but no maximum, and some of the estimates for UTK are outside the range for the county-wide data.


```{r plotsaliva100k, echo=FALSE, message=FALSE, warning=FALSE}
saliva_plot <- ggplot(saliva_data, aes(x=DATE, y=New_cases_per_100k)) + 
geom_rect(mapping=aes(xmin=min(saliva_data$DATE), xmax=max(saliva_data$DATE), ymin=0, ymax=1), fill="darkolivegreen1") +
  geom_rect(mapping=aes(xmin=min(saliva_data$DATE), xmax=max(saliva_data$DATE), ymin=1, ymax=10), fill="khaki1") +
  geom_rect(mapping=aes(xmin=min(saliva_data$DATE), xmax=max(saliva_data$DATE), ymin=10, ymax=25), fill="tan1") +
  geom_rect(mapping=aes(xmin=min(saliva_data$DATE), xmax=max(saliva_data$DATE), ymin=25, ymax=max(saliva_data$New_cases_per_100k)), fill="indianred1") + 
geom_point() + ylab("Est. daily new cases 100,000 people based on UTK saliva samples") + xlab("Date") + ylim(0,NA) + scale_colour_viridis_d(end=0.8) 
print(saliva_plot)
```
