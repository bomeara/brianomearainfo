...
Tennessee now breaks out covid test results for school age children (age 5-18). The number of tests in this age group and the underlying population size aren't known, so things like positivity rate and proportion of students infected cannot be determined, but the raw number of students infected can be shown:

```{r studentinfections, echo=FALSE, message=FALSE, warning=FALSE}
try(student_covid_total <- ggplot(schoolkids_daily, aes(x=DATE, y=CASE_COUNT, group=Region)) + geom_line(aes(colour=Region)) +  ylab("Number of students with positive covid results") + xlab("Date") + scale_colour_viridis_d(end=0.8))
