...
```{r greenzone, echo=FALSE, message=FALSE, warning=FALSE}
...
Another way to look at testing (or disease spread) is to look at the proportion of positive tests: they can go up if a greater proportion of symptomatic people are being tested and/or if the frequency of the disease is increasing. WHO recommends testing should be 10% positive or lower (black line).
...
for(i in seq_along(webfiles)) {
  ...
  raw <- gsub('/span/tdtd style\"width: 50.0000%;\"span style\"font-size: 18px;\"', '', raw, fixed=TRUE)
  raw <- gsub('/span/tdtd style\"width: 50%; text-align: left;\"span style\"font-size: 18px;\"', '', raw, fixed=TRUE)
...
utk_plot <- ggplot(utk.cases, aes(x=date, y=count, group=group)) + geom_line(aes(colour=group)) + geom_point(aes(colour=group)) + ylab("Number of active cases at UTK") + xlab("Date") + ylim(0,NA) + scale_colour_viridis_d(end=0.8)
print(utk_plot)
```
...
Another question is people doing searching. From Google Trends:

<script type="text/javascript" src="https://ssl.gstatic.com/trends_nrtr/2213_RC01/embed_loader.js"></script> <script type="text/javascript"> trends.embed.renderExploreWidget("TIMESERIES", {"comparisonItem":[{"keyword":"covid symptoms","geo":"US-TN-557","time":"today 3-m"}],"category":0,"property":""}, {"exploreQuery":"date=today%203-m&geo=US-TN-557&q=covid%20symptoms","guestPath":"https://trends.google.com:443/trends/embed/"}); </script> 
...
Currently, regional hospitals have `r current.capacity.df['ICU Beds', 'Available']` ICU beds available of `r current.capacity.df['ICU Beds', 'Total.Capacity']` total, and `r current.capacity.df['Ventilators', 'Available']` available ventilators out of `r current.capacity.df['Ventilators', 'Total.Capacity']` total. When a line hits 100%, the local hospitals are theoretically full for that resource (for all patients, not just covid patients), though there is surge capacity on top of this. Note that these data are updated only once a week: I use a dot at the first point for a new set of data.


```{r hospitalcapacityplot, echo=FALSE, message=FALSE, warning=FALSE}
hosp_plot <- ggplot(capacity.df, aes(x=Date, y=Current.Utilization, group=Resource)) + geom_line(aes(colour=Resource)) + geom_point(aes(colour=Resource)) + ylab("Percent Utilization in East Tennessee Region") + xlab("Date") + ylim(0,100) + scale_colour_viridis_d(end=0.8)
print(hosp_plot)
