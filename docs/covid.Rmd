...
GetZone <- function(cases) {
  ...
  max.allowed.green <- mean(old.cases)+1.5*sd(old.cases)
  max.allowed.yellow <- mean(old.cases)+3*sd(old.cases)
  result <- "red"
  if(mean(new.cases)<max.allowed.yellow) {
    result <- "yellow"
  }
  if(mean(new.cases)<max.allowed.green) {
    result <- "green"
  }
  return(result)
}
