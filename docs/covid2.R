...
try(student_covid_daily <- ggplot(schoolkids_daily, aes(x=DATE, y=NEW_CASES, group=Region)) + geom_point(aes(colour=Region), size=0.5)+ geom_ma(aes(colour=Region, linetype="a"), n=7) +  ylab("Number of students with new positive covid results daily") + xlab("Date") + scale_colour_viridis_d(end=0.8))
